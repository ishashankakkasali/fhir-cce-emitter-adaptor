...
| `ResourceEnricher` | Path-based enrichment: locates RelatedPerson via configured `related-person-paths`, resolves national-id, sets `subject.reference = Patient/<national-id>`. Strict — returns null (skip forward) if any step fails. |
...
| `EMITTER_RELATED_PERSON_PATHS` | `ResourceType:dot.path` entries for locating RelatedPerson references | *(4 defaults)* |
