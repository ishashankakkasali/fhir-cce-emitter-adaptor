...
└── service/
    ...
    ├── ReferenceResolver.java                    # Resolves national-id from FHIR resources: path lookup, identity-source detection, fetch, multi-strategy matching; Practitioner display name resolution
    ├── ResourceEnricher.java                     # Enriches FHIR JSON: subject.reference, patient.reference, OR identifier[] based on resource type; Practitioner display name enrichment
